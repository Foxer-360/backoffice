...
export interface AddComponentObject {
  id?: number;
  name: string;
  position?: number;
  data: LooseObject;
...
export interface ComponentsServiceLikeClass {
...
export interface PluginServiceLikeClass {
...
export interface Properties {
  ...
  locked?: number[];
  componentService: ComponentsServiceLikeClass; // Service for components
  pluginService: PluginServiceLikeClass; // Service for plugins
  ...
  // Event handlers
  onComponentAdded?: (data: LooseObject) => void;
  onComponentTryAdd?: (data: LooseObject) => void;
  onComponentRemoved?: (id: number) => void;
  onComponentTryRemove?: (id: number) => void;
  onComponentUpdated?: (id: number, data: LooseObject) => void;
  onComponentTryUpdate?: (id: number, data: LooseObject) => void;
  ...
  // Activators
  activateComponentAdd?: (data: LooseObject) => Promise<boolean>;
  activateComponentRemove?: (id: number) => Promise<boolean>;
  activateComponentUpdate?: (id: number, data: LooseObject) => Promise<boolean>;
  ...
  activateCommit?: (data: LooseObject) => Promise<boolean>;
}
...
export interface State {
  // content: ComponentObject[]; // Content composed from components
  // contentNew: LooseObject;
  content: IContent;
  data: LooseObject; // Data from each plugin
  ...
  componentEditor: {
    ...
    revData: LooseObject; // Reverse data. If user cancel his updates, these data will be pushed into component
  };
}

class Composer extends Component<Properties, State> {
        ...
        >
          ...
          >
            <div style={{ position: 'relative' }}>
              <Spinner ref={(node) => this.spinner = node}>
                <Editor
   ...
   * @return {void}
   */
  public setPluginData(name: string, data: LooseObject): void {
    this.setState({
      data: {
        ...this.state.data,
        [name]: data
   ...
   *
   * @param {AddComponentObject} data of new component
   * @param {boolean} activated? true if you want to skip activator in addComponent flow
   * @return {Promise<void>}
   */
  public addComponent(data: AddComponentObject, activated?: boolean): Promise<boolean> {
  ...
  public updateComponent(id: number, data: ComponentObject, activated?: boolean): Promise<boolean> {
  ...
  public updatePositions(positionMap: LooseObject, activated?: boolean): Promise<boolean> {
   ...
   * @return {void}
   */
  private eventCommponentTryAdd(data: AddComponentObject): void {
   ...
   * @return {void}
   */
  private eventComponentAdded(data: AddComponentObject): void {
    // Just call event if exists
   ...
   * @param {ComponentObject} data updated data of component
   * @return {void}
   */
  private eventComponentTryUpdate(id: number, data: LooseObject): void {
    // Just call event if exists
   ...
   * @return {void}
   */
  private eventComponentUpdated(id: number, data: ComponentObject): void {
    // Just call event if exists
   ...
   */
  private activateComponentAdd(data: LooseObject): Promise<boolean> {
   ...
   * @param {ComponentObject} data of component
   * @return {Promise<boolean>}
   */
  private activateComponentUpdate(id: number, data: LooseObject): Promise<boolean> {
   ...
   */
  private activateCommit(data: LooseObject): Promise<boolean> {
   ...
   *
   * @param {LooseObject} data in component
   * @return {void}
   */
  private _eventUpdateComponent(data: LooseObject): void {
    ...
    // const component = this.state.content.find((c: ComponentObject) => {
    //   if (c.id === id) {
    //     return true;
    //   }
   ...
   * @return {Promise<boolean>}
   */
  private handleAddComponent(data: LooseObject, position: number,
    ...
    // // Get new ID
    // let id = 0;
    // this.state.content.forEach((o: ComponentObject) => {
    //   if (o.id >= id) {
    //     id = o.id + 1;
    //   }
    // });
    ...
    //   const content = this.state.content.map((o: ComponentObject) => {
  ...
  private handleRemoveComponent(id: number, activated?: boolean): Promise<boolean> {
    ...
    // let position = -1;
    // let index = -1;
    // this.state.content.forEach((o: ComponentObject, i: number) => {
    //   if (o.id === id) {
    //     position = o.position;
    //     index = i;
    //   }
    ...
    //     let content = this.state.content.map((o: ComponentObject) => {
    //       if (o.position > position) {
    //         o.position--;
    //       }
   ...
   * @param {LooseObject} data of component (inner data without name, id, position etc)
   * @param {boolean} activated? true if you want to skip activator
   * @return {Promise<boolean>}
   */
  private handleUpdateComponent(id: number, data: LooseObject, activated?: boolean): Promise<boolean> {
    // Using delta
    // tslint:disable-next-line:no-console
  ...
  private handleStartEditComponent(id: number, activated?: boolean): Promise<boolean> {
    // tslint:disable-next-line:no-any
    const elm = getObjectFromContent(this.state.content, id + '') as LooseObject;
    ...
    // let data = null as LooseObject;
    // let found = false;
    // this.state.content.forEach((c: ComponentObject) => {
    //   if (c.id === id) {
   ...
   * @return {Promise<boolean>}
   */
  private handleStopEditComponent(id?: number, activated?: boolean): Promise<boolean> {
    ...
    return activator.then((can: boolean) => {
      ...
      return new Promise((resolve) => {
        this.setState({
          ...
          this.eventComponentStopEdit(id);
          this.spinner.disable();
          resolve(true);
        });
      }) as Promise<boolean>;
   ...
   *
   * @param {LooseObject} positionMap where key id id of component and value is position
   * @param {boolean} activated? true if you want to skip activator
   * @return {Promise<boolean>}
   */
  private handleUpdatePositions(positionMap: LooseObject, activated?: boolean): Promise<boolean> {
    return Promise.resolve(true);
    ...
    //   const content = this.state.content.map((c: ComponentObject) => {
   ...
   * @return {void}
   */
  private handlePluginDataChange(name: string, data: LooseObject): void {
    this.setState({
      data: {
        ...this.state.data,
        [name]: data
  ...
  private handleRevertComponent(): Promise<boolean> {
    ...
    const component = getObjectFromContent(this.state.content, id + '') as LooseObject;
  ...
  /**
   * Comparator method for sort function to sort components in
   * content
   *
   * @param {ComponentObject} left
   * @param {ComponentObject} right
   * @return {number}
   */
  private compareComponents(left: ComponentObject, right: ComponentObject): number {
